...
exports.login = function( req, username, password, done ){
	if( ! username || ! password ){
		...
		req.flash(
			'message',
			'Need a ' + ! username && 'username' || ! password && 'password' + ' to authenticate.' )
		);
	}
	...
	var verifyAccount = function( err, account ){
		...
		if ( ! account.validPassword( password ) ){
			...
			return account.authenticate( account, req, done, 'Login successfull!' );
		}
	}
...
exports.register = function( req, username, password, done ){
	...
	var saveAccount = function( err, user ){
		...
		return done( null, false, req.flash( 'message', 'That username is already taken, try another one.' ) );
	};

	// asynchronous
	process.nextTick(function() {
		if( ! req.user ){
			return Account.findOne( fields, saveAccount );
		}
